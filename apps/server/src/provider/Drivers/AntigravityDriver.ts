...
const decode = Schema.decodeSync(AntigravitySettings);
...
export const AntigravityDriver: ProviderDriver<AntigravitySettings, AntigravityDriverEnv> = {
  ...
  metadata: { displayName: "Antigravity", supportsMultipleInstances: false },
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      let lastSuccessfulModels: ReadonlyArray<ServerProviderModel> | undefined;
        ...
        Effect.tap((checked) =>
          Effect.sync(() => {
            if (checked.status === "ready") {
              lastSuccessfulModels = checked.models.filter((model) => !model.isCustom);
            }
          }),
        ),
