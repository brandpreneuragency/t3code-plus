...
export const CommandCodeSettings = makeProviderSettingsSchema(
  {
    // Off by default like Cursor, Grok, and OpenCode until the binding is
    // proven on a given install. Users opt in from Settings.
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(false)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
    binaryPath: makeBinaryPathSetting("command-code").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the Command Code CLI. On Windows the short alias is cmdc.",
...
export const AntigravitySettings = makeProviderSettingsSchema(
  {
    // Experimental binding: never probe or enable it until a user opts in.
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(false)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
...
export const ServerSettings = Schema.Struct({
  ...
  /**
   * Base URL of the llm-monitor instance backing the model catalogue page,
   * e.g. `https://models.brandpreneur.net`. Empty disables the feature and
   * hides its nav entry. The matching credential lives in the secret store
   * under `modelCatalogue.basicAuth`, never here.
   */
  modelCatalogueUrl: TrimmedString.pipe(Schema.withDecodingDefault(Effect.succeed(""))),
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  modelCatalogueUrl: Schema.optionalKey(TrimmedString),
